# Advent of Code 2019 day 7 part B
...
memory_set = [list(init_memory),list(init_memory),list(init_memory),list(init_memory),list(init_memory)]

def run_intcode(phase_setting, INPUT, memory_index):

    # print("input",INPUT)
    # print("phase",phase_setting)

    # grab memory
    memory = memory_set[memory_index]
...
def run_sequence(seq, INPUT):
    ans1 = run_intcode(seq[0], INPUT, 0)
    ans2 = run_intcode(seq[1], ans1 , 1)
    ans3 = run_intcode(seq[2], ans2 , 2)
    ans4 = run_intcode(seq[3], ans3 , 3)
    ans5 = run_intcode(seq[4], ans4 , 4)
    return ans5

# generate permutations of the phase setting sequences 
max_thruster_signal = 0 
comb = list(itertools.permutations([5,6,7,8,9],5))
